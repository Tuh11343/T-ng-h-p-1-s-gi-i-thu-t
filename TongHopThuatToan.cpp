...
void Merge(int a[], int l, int m, int r)
{
    ...
    int L[n1], R[n2]; // khởi tạo 2 mảng tạm

    // sao chép các phần tử qua mảng tạm
    for (int i = 0; i < n1; i++)
        L[i] = a[l + i];
    for (int j = 0; j < n2; j++)
        R[j] = a[m + 1 + j];

    int i = 0, j = 0, k = l; // khởi tạo vị trí của các mảng

    // Bắt đầu tiến hành sắp xếp
    while (i < n1 && j < n2)
    {
        if (L[i] < R[j])
        {
...
{
    int b[n];
    int MaxIndex = getMax(a, n);
    int Count[MaxIndex] = {0};
    for (int i = 0; i < n; i++) // Tăng biến đếm của giá trị
        Count[a[i]]++;
    ...
    for (int i = 0; i < n; i++) // truyền lại dữ liệu vào mảng ban đầu
        a[i] = b[i];
}
